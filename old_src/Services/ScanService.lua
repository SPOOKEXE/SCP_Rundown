...
function ScanEventClass:WormToPosition( origin, goal, callback )
	local wormUID = HttpService:GenerateGUID(false)
	...
	if IsFarDistance then
		...
		PathObject:ComputeAsync( origin, goal )
		local Waypoints = PathObject:GetWaypoints()
		ScanService.Client.onScanWormCreated:FireAll(wormUID, origin, Waypoints, goal, false)
		for _, waypoint in ipairs( Waypoints ) do
			task.wait(0.25)
		end
	end
	if typeof(callback) == 'function' then
		task.spawn(callback, wormUID)
	end
end
		...
		for _, tbl in ipairs( currentScanData ) do
			...
			for _ = 1, tbl.NodeCount do
				if self.ScanIndex == 1 then
					...
					task.spawn(function()
						self:WormToPosition(self.Model:GetPrimaryPartCFrame().Position, RandomNode.Position, function(portalUID)
							ScanService:CreateScanPortal(portalUID, RandomNode, self, tbl, function(_)
								ScanService.Client.onScanSpotEnded:FireAll(portalUID)
							end)
						end)
					end)
					...
					task.spawn(function()
						self:WormToPosition( self.LastNodePositions[self.LastNodeIndex], RandomNode.Position, function(portalUID)
							ScanService:CreateScanPortal(portalUID, RandomNode, self, tbl, function(_)
								ScanService.Client.onScanSpotEnded:FireAll(portalUID)
								table.insert(self.LastNodePositions, RandomNode.Position)
...
-- // Module // --
function ScanService:CreateScanPortal( portalUID, NodePart, scanClass, scanDataTable, Callback )
	...
	local positionCFrame = CFrame.new( positionVector ) * CFrame.Angles(math.rad(-90), 0, 0)
	ScanService.Client.onScanSpotCreated:FireAll(portalUID, ScannerModel, positionCFrame)

	-- clear everything but primary part
	ScannerModel = ScannerModel:Clone()
	for _, Part in ipairs( ScannerModel:GetChildren() ) do
		if Part:IsA('BasePart') and ScannerModel.PrimaryPart ~= Part then
			Part:Destroy()
		end
	end
	ScannerModel:SetPrimaryPartCFrame( positionCFrame )
